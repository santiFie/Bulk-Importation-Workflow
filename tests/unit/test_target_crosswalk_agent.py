"""
Tests unitarios para el agente de generación de crosswalk hacia SEDICI/DSpace.

Verifica:
  - Carga y estructura del catálogo formal de metadatos de SEDICI.
  - Nivel 1: Mapeo determinista por composición de campos genéricos conocidos.
  - Identificación de columnas remanentes.
  - Nivel 3: Guardrail determinista contra la lista blanca de SEDICI.
  - Serialización y fusión del archivo JSON de configuración.
  - Ejecución integral del nodo generate_sedici_target_crosswalk_config.
"""
...
def sample_pubmed_csv(tmp_path):
    """Genera un CSV de prueba con la estructura exacta de PubMed."""
...
class TestSediciCatalog:
    ...
    def test_get_valid_fields_helper(self, sample_catalog):
        ...
        assert "sedici.identifier.other" not in allow_lang  # Los identificadores no llevan [es]
...
class TestConfigSerializationAndNode:
    ...
    @pytest.mark.asyncio
    async def test_export_subgraph_compilation_and_topology(self):
        """Verifica que ExportSubgraph compile con el nuevo nodo GenerateSediciTargetConfig."""
