# Test de Evaluación del Pipeline — Resumen de Cambios

## Archivos modificados/creados

### 1. [graph.py](file:///home/santi/Documentos/LangGraph/Modulo-Marta/core/graph.py) — Modificado
...
async def create_graph(persistence_saver):
    ...
    graph.add_node("GenerateSafToImport", generate_saf_to_import)  # Paso 8
    graph.add_node("ImportToDspace", import_to_dspace)           # Paso 9
    ...
    graph.add_edge("GenerateSafToImport", "ImportToDspace")      # Paso 9
...
PIPELINE_STEPS: list[tuple[str, str]] = [
    ...
    ("Paso 8 - GenerateSafToImport",           "GenerateSafToImport"),
    ("Paso 9 - ImportToDspace",                "ImportToDspace"),
]
"""
...
**Qué se agregó:**
- **`PIPELINE_STEPS`** — registro ordenado `[(etiqueta, nombre_nodo)]` de los 9 pasos del pipeline
- **`get_step_node_names()`** — devuelve la lista de nombres de nodos en orden
- **`get_step_label()`** — dado un nombre de nodo, devuelve su etiqueta legible
- **`_NODE_FUNCTIONS`** — mapeo `{nombre_nodo: función}` para ejecución directa
- **`run_pipeline_until_step(state, stop_after)`** — ejecuta el pipeline secuencialmente hasta el paso indicado (inclusive), capturando errores por paso

> [!IMPORTANT]
> `run_pipeline_until_step()` ejecuta las funciones de nodo directamente (sin compilar un subgrafo), lo que es más predecible para testing. Si un nodo falla, la ejecución se detiene y el error queda registrado en `results[nodo]["__error__"]`.

---

### 2. [test_pipeline_evaluation.py](file:///home/santi/Documentos/LangGraph/Modulo-Marta/tests/test_pipeline_evaluation.py) — Nuevo

**Test de evaluación con LangSmith** siguiendo el mismo patrón que `test_source_config_generator.py`:

| Sección | Descripción |
|---------|-------------|
| `TEST_CASES` | Lista de CSVs a evaluar (configurable, soporta múltiples CSVs) |
| `STOP_AFTER_STEP` | Paso en el que se detiene la ejecución |
| `predict_pipeline()` | Función objetivo que ejecuta el pipeline y recopila info de cada paso |
| 7 evaluadores | Métricas heurísticas por cada aspecto del pipeline |

**Evaluadores implementados:**

| Evaluador | Qué mide |
|-----------|----------|
| `no_errors` | Ningún paso lanzó excepción |
| `all_steps_executed` | Se ejecutaron todos los pasos esperados |
| `files_generated` | Los archivos intermedios esperados existen |
| `row_count` | Los CSVs tienen al menos 1 fila |
| `generic_columns` | El CSV genérico tiene las columnas esperadas (title, author, etc.) |
| `sedici_columns` | El CSV SEDICI-ready tiene las columnas esperadas |
| `data_integrity` | Las filas no crecen entre pasos sucesivos |

---

### 3. [run_pipeline.py](file:///home/santi/Documentos/LangGraph/Modulo-Marta/tests/studio/run_pipeline.py) — Nuevo

**Script de ejecución manual** para LangGraph Studio, con:
- Salida formateada con colores (✓/✗ por paso)
- Argumento CLI `--stop-after` para ejecutar hasta un paso específico
- Resumen de archivos generados (filas, columnas)
- Visualización clara de errores con traceback

```bash
# Ejecución completa hasta MetadataCorrections
python tests/studio/run_pipeline.py

# Ejecutar solo hasta Deduplicate
python tests/studio/run_pipeline.py --stop-after Deduplicate
```

---

## Cómo agregar más CSVs de prueba

Editar `TEST_CASES` en [test_pipeline_evaluation.py](file:///home/santi/Documentos/LangGraph/Modulo-Marta/tests/test_pipeline_evaluation.py):
...
TEST_CASES = [
    {
        ...
        "expected_sedici_columns": ["dc.title[es]"],
    },
    {  # Segundo caso de prueba
        "source_csv_path": os.path.join(DATA_DIR, "otro_csv.csv"),
        "source_name": "scopus",
        "repository_csv_path": os.path.join(DATA_DIR, "export_10915_all.csv"),
        "expected_generic_columns": ["title", "author"],
        "expected_sedici_columns": [],
    },
]
```

## Cómo cambiar el paso de corte

Editar `STOP_AFTER_STEP` o usar el CLI:

```python
# En test_pipeline_evaluation.py
STOP_AFTER_STEP = "Deduplicate"  # Solo pasos 1, 2a, 2b, 3
```

```bash
# En run_pipeline.py
python tests/studio/run_pipeline.py --stop-after MapSourceToGeneric
```
